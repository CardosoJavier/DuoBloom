...
-- ---------------------------------------------
CREATE TABLE
    IF NOT EXISTS "nutrition_streaks" (
...
-- Policy: View (SELECT)
-- Users can see their own streak and their partner's streak
CREATE POLICY "Users can view own and partner streak state" ON "nutrition_streaks" FOR
SELECT
    USING (
        auth.uid () = user_id
        OR EXISTS (
            ...
            WHERE
                (
                    ...
                    AND user_two_id = nutrition_streaks.user_id
                )
                OR (
                    user_two_id = auth.uid ()
                    AND user_one_id = nutrition_streaks.user_id
                )
        )
    );

-- Policy: Insert / Update (UPSERT)
-- Users can upsert their own streak state row
CREATE POLICY "Users can upsert own streak state" ON "nutrition_streaks" FOR INSERT
WITH
    CHECK (auth.uid () = user_id);

CREATE POLICY "Users can update own streak state" ON "nutrition_streaks" FOR UPDATE
...
-- Triggered by the client when a retroactive log is inserted (log_date < last_streak_day).
-- Loops through ALL nutrition_logs for the user sorted ascending, rebuilds current_streak_count
-- and all_time_streak_count, then upserts the result into nutrition_streaks.
...
BEGIN
    ...
    IF v_dates IS NULL OR array_length(v_dates, 1) = 0 THEN
        -- No logs at all — reset the streak row
    ...
    -- If the last log day is neither today nor yesterday, the streak is currently broken
...
-- Grant execute to authenticated users so the client can call it via RPC
